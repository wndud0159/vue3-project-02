<template>
    <div class="flex flex-col">
                ...
                <div class="flex items-center justify-center">
                    <div class="absolute right-0 top-3">
                        <router-link
                            ...
                            v-if="currentUser"
                            :class="`${
                                router.currentRoute.value.path == '/myinfo' ? 'text-primary' : ''
                            }  flex items-center space-x-1 hover:text-primary text-gray-500 hover:opacity-80 cursor-pointer z-50`"
                        >
                            <img :src="currentUser.profile_image_url" class="w-12 h-12 rounded-full object-cover" />
                ...
                <div class="w-4/5 text-gray-500 text-lg">
                    <div class="flex">
                        <router-link
                            to="/"
                            :class="`${
                                router.currentRoute.value.path == '/' ? 'text-primary' : ''
                            }   flex items-center justify-center py-6 border-b-2    w-1/4 cursor-pointer border-gray-100 hover:border-primary hover:text-primary`"
                        >
                            <span>홈</span>
                        </router-link>
                        <div @click="onIbackIntroduce" :class="`flex items-center justify-center py-6  border-b-2    w-1/4 cursor-pointer  border-gray-100 hover:border-primary hover:text-primary`">
                            <span>아이백이란?</span>
                        </div>
                        <router-link
                            to="/servicecenter"
                            :class="`${
                                router.currentRoute.value.path == '/businessalliance' ? 'text-primary' : ''
                            }   flex items-center justify-center py-6  border-b-2    w-1/4 cursor-pointer border-gray-100 hover:border-primary hover:text-primary`"
                        >
                            <span>고객센터</span>
                        </router-link>
        ...
        <div class="fixed flex text-center items-center md:hidden border-t-2 justify-center border-gray-100 mt-20 bottom-0 w-full bg-white text-gray-400 z-30">
            ...
            <router-link to="/" :class="` w-1/4 py-3 flex flex-col space-y-1  ${router.currentRoute.value.path == '/' ? 'text-primary' : ''} cursor-pointer hover:text-primary`">
                <i class="fas fa-home text-xl"></i>
                <span class="text-base">홈</span>
            </router-link>
            <div
                @click="onIbackIntroduce"
            ...
            <router-link to="/myinfo" v-if="currentUser" :class="` w-1/4 py-3 flex flex-col space-y-1 cursor-pointer ${router.currentRoute.value.path == '/myinfo' ? 'text-primary' : ''} `">
                <i class="fas fa-user text-xl"></i>
                <span class="text-base">내정보</span>
            </router-link>
            <router-link to="/login" v-if="!currentUser" :class="` w-1/4 py-3 flex flex-col space-y-1 cursor-pointer hover:text-primary`">
                <i class="fas fa-user text-xl"></i>
                <span class="text-base">내정보</span>
            </router-link>
            <!-- </div> -->
        ...
        <div v-if="currentUser" :class="`${sideToggleMenu ? 'md:w-2/5 w-full' : 'w-0'} fixed right-0 bg-white  h-full  z-50 transition-all`">
            ...
            <div v-if="sideToggleMenu" class="flex flex-col">
                <div class="flex justify-center">
                    <img class="w-24 h-24 border-4 border-white rounded-full" :src="currentUser.profile_image_url" alt="" />
                </div>
              ...
              나의 체크리스트 <i class="fas fa-angle-right text-gray-400 "></i></button> -->
                    <router-link
                        to="/notice"
                        @click="sideToggleMenu = false"
                        class="py-5 flex items-center w-full md:px-6 px-3 justify-between border-b border-gray-100 hover:bg-gray-100 cursor-pointer"
                    >
                        공지사항 <i class="fas fa-angle-right text-gray-400"></i
                    ></router-link>
                    <router-link
                        to="/termsofuse"
                        @click="sideToggleMenu = false"
                        class="py-5 flex items-center w-full md:px-6 px-3 justify-between border-b border-gray-100 hover:bg-gray-100 cursor-pointer"
                    >
                        서비스 이용약관 <i class="fas fa-angle-right text-gray-400"></i
                    ></router-link>
                    <router-link
                        to="/privacypolicy"
                        @click="sideToggleMenu = false"
                        class="py-5 flex items-center w-full md:px-6 px-3 justify-between border-b border-gray-100 hover:bg-gray-100 cursor-pointer"
                    >
                        개인정보 보호정책 <i class="fas fa-angle-right text-gray-400"></i
                    ></router-link>

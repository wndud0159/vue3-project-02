...
<div class="flex flex-col items-center px-3 mb-8">
    ...
    <div class="w-full flex flex-col justify-center items-center rounded-sm pt-8 pb-8">
        ...
        <!-- question section -->
        <div v-if="health.length" class="flex flex-col items-center text-2xl font-semibold w-full md:w-96">
            <div v-if="step1" class="flex flex-col items-center ">
                <div class="mb-8 ">1. {{health[0].question}}</div>
                <div class="w-full flex flex-col items-center text-lg text-white bg-lighter rounded-xl px-3 py-3 -mt-2">
                    <div>답변 예시 :</div>
                    <div>제 남편이 저 대신 결정을 내리면 좋겠어요. 저는 제 동생이 결정에 관여하는 것을 원치 않아요.</div>
                </div>
            </div>
            <div v-if="step2" class="flex flex-col items-center ">
                <div class="mb-8 ">2. {{health[1].question}}</div>
                <div class="w-full flex flex-col items-center text-lg text-white bg-lighter rounded-xl px-3 py-3 -mt-2">
                    <div>답변 예시 :</div>
            ...
            <div v-if="step3" class="flex flex-col items-center ">
                <div class="mb-8 ">3. {{health[2].question}}</div>
                <div class="w-full flex flex-col items-center text-lg text-white bg-lighter rounded-xl px-3 py-3 -mt-2">
                    <div>답변 예시 :</div>
                    <div>사전연명치료금지 의향서 작성,  의사와 연명의료계획서 작성, 김영수를 성년후견인 지정함 등</div>
                    <div class=" text-blue-500 underline">사전연명치료금지 알아보기</div>
                </div>
            </div>
            <div v-if="step4" class="flex flex-col items-center ">
                <div class="mb-8 ">4. {{health[3].question}}</div>
                <div class="w-full flex flex-col items-center text-lg text-white bg-lighter rounded-xl px-3 py-3 -mt-2">
    ...
    <div v-if="step2 && health.length" class="w-full">
        ...
        <!-- text section -->
        <div class="w-full flex justify-center mb-8">
            <textarea v-model="health[1].answer_text" placeholder="희망사항" class=" w-full resize-none md:w-96 outline-none py-2 px-2  focus:border-primary border focus:ring-4 focus:ring-primary focus:ring-opacity-25 ring-gray-300  rounded-md border-gray-300"
             rows="4">
            </textarea>
        </div>
    </div>
    ...
    <div v-if="step3 && health.length" class="w-full">
        ...
        <!-- text section -->
        <div class="w-full flex justify-center mb-8">
            <textarea v-model="health[2].answer_text" placeholder="희망사항" class=" w-full md:w-96 resize-none outline-none py-2 px-2  focus:border-primary border focus:ring-4 focus:ring-primary focus:ring-opacity-25 ring-gray-300  rounded-md border-gray-300"
            rows="4">
            </textarea>
        </div>
    </div>
    ...
    <div v-if="step4 && health.length" class="w-full">
        ...
        <div class="text-xl mb-8 flex flex-col items-center w-full space-y-3">
            <div class="bg-gray-100 px-3 md:w-96 py-3 flex items-center space-x-2 w-full">
                <input v-model="health[3].answer_box" class="w-8 h-8" type="radio" value="나는 장기기증을 희망합니다. 등록기관에 제출한 장기기증 문서를 참고해주세요."><span>네</span>
            </div>
            <div class="bg-gray-100 px-3 md:w-96 py-3 flex items-center space-x-2 w-full">
